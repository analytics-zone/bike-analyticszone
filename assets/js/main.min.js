...
const dropdownToggles=document.querySelectorAll('.dropdown-toggle');dropdownToggles.forEach(toggle=>{toggle.addEventListener('click',function(e){const isMobile=window.innerWidth<=768;if(isMobile){e.preventDefault();const parentLi=this.closest('.has-submenu');const submenu=parentLi.querySelector('.submenu');const isExpanded=this.getAttribute('aria-expanded')==='true';dropdownToggles.forEach(otherToggle=>{if(otherToggle!==this){otherToggle.setAttribute('aria-expanded','false');const otherParent=otherToggle.closest('.has-submenu');const otherSubmenu=otherParent.querySelector('.submenu');if(otherSubmenu){otherSubmenu.style.display='none';}}});this.setAttribute('aria-expanded',!isExpanded);if(submenu){submenu.style.display=isExpanded?'none':'block';}}});});const languageSelect=document.getElementById('language-select');if(languageSelect){languageSelect.addEventListener('change',function(){const newLang=this.value;const currentPath=window.location.pathname;const pathParts=currentPath.split('/').filter(part=>part);const currentLang=pathParts[0];let newPath=currentPath.replace('/'+currentLang+'/','/'+newLang+'/');window.location.href=newPath;});}
...
sessionStorage.setItem('splashShown','true');document.body.classList.add('splash-active');const splashScreen=document.createElement('div');splashScreen.className='splash-screen';splashScreen.innerHTML=`
            <div class="splash-logo">
                <img src="/assets/images/logo.png" alt="BikeAnalytics" width="200" height="200">
                <div class="splash-logo-text">BikeAnalytics</div>
            </div>
        `;document.body.insertBefore(splashScreen,document.body.firstChild);setTimeout(function(){document.body.classList.remove('splash-active');const header=document.querySelector('.site-header');const mainContent=document.querySelector('.main-content');if(header){header.classList.add('content-reveal');}
